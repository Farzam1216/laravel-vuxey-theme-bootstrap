...
$(selector).signature({color: 'blue', guideline: true}) */
  ...
  console.log('Signature changed');
		...
		_create: function() {
			...
			try {
				this.canvas = $('<canvas id="sketchpad" width="' + this.element.width() + '" height="' +
					this.element.height() + '">' + this.options.notAvailable + '</canvas>')[0];
				this.element.append(this.canvas);

                // this.canvas = $('<canvas id="sketchpad" width="' + this.element.width() + '">' + this.options.notAvailable + '</canvas>')[0];
				// this.element.append(this.canvas);
			}
			catch (e) {
				$(this.canvas).remove();
